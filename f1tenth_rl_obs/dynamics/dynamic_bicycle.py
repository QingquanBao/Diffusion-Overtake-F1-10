from f1tenth_rl_obs.dynamics import Dynamics
...
class DynamicBicycle(Dynamics, nn.Module):
    ...
    def calculate_tire_forces(self, states, control_inputs):
        ...
        if batch_mode:
            ...
            alpha_f = states[:, STEERING_ANGLE] - torch.arctan(
                (states[:, YAW_RATE] * self.lf + states[:, VY]) / states[:, VX])
    ...
    def forward(self, states, control_inputs):
        ...
        if batch_mode:
            ...
            diff[:, VX] = 1.0 / self.mass * (
                        tire_forces[:, FRX] - tire_forces[:, FFY] * torch.sin(states[:, STEERING_ANGLE]) + states[:,
                                                                                                           VY] * states[
                                                                                                                 :,
                                                                                                                 YAW_RATE] * self.mass)
            diff[:, VY] = 1.0 / self.mass * (
                        tire_forces[:, FRY] + tire_forces[:, FFY] * torch.cos(states[:, STEERING_ANGLE]) - states[:,
                                                                                                           VX] * states[
                                                                                                                 :,
                                                                                                                 YAW_RATE] * self.mass)
            diff[:, YAW_RATE] = 1.0 / self.Iz * (
                        tire_forces[:, FFY] * self.lf * torch.cos(states[:, STEERING_ANGLE]) - tire_forces[:,
                                                                                               FRY] * self.lr)
            ...
            diff[YAW] = states[YAW_RATE]
            diff[VX] = 1.0 / self.mass * (
                        tire_forces[FRX] - tire_forces[FFY] * torch.sin(states[STEERING_ANGLE]) + states[VY] * states[
                    YAW_RATE] * self.mass)
            diff[VY] = 1.0 / self.mass * (
                        tire_forces[FRY] + tire_forces[FFY] * torch.cos(states[STEERING_ANGLE]) - states[VX] * states[
                    YAW_RATE] * self.mass)
            diff[YAW_RATE] = 1.0 / self.Iz * (
                        tire_forces[FFY] * self.lf * torch.cos(states[STEERING_ANGLE]) - tire_forces[FRY] * self.lr)
            diff[STEERING_ANGLE] = control_inputs[STEER_SPEED]
        return diff